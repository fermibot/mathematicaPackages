...
Module[{database, data, dataH, yMin = 0, yMax = 500, ySteps = 20},
  database =
      OpenSQLConnection[
...
codes\\fitBitDataAnalysis\\activeMinutes.sql"],
  ...
  data = GroupBy[{DateObject@#[[1]], #[[2]], #[[3]]} & /@
      data, #[[3]] &];
  data = #[[All, ;; 2]] & /@ data;
  DateListPlot[data, ImageSize -> 1800, AspectRatio -> 0.2,
    Frame -> True,
    FrameTicks -> {data[[1, All, 1]][[;; ;; 15]],
      Range[yMin, yMax, ySteps]},
    DateTicksFormat -> { "Day", "/", "MonthNameShort", "/", "Year"},
    GridLines -> {data[[1, All, 1]][[;; ;; 15]],
      Range[yMin, yMax, ySteps]}, FrameLabel -> {"", "Time in Minutes"},
...
Module[{database, data, dataH, yMin = 0, yMax = 4000, ySteps = 200},
  database = OpenSQLConnection["fitbit"];
  data = SQLExecute[database,
    Import["D:\\Programming\\mathematicaPackages\\mathematica_notebook_\
...
Module[{database, data, dataH, yMin = 10000, yMax = 20000,
  ySteps = 500, keys},
  database = OpenSQLConnection["fitbit"];
  data = SQLExecute[database,
    Import["D:\\Programming\\mathematicaPackages\\mathematica_notebook_\
codes\\fitBitDataAnalysis\\caloriesByWeek.sql"]];
...
Module[{database, data, dataH, yMin = 0, yMax = 100000, ySteps = 5000,
  keys},
  database = OpenSQLConnection["fitbit"];
  data = SQLExecute[database,
    Import["D:\\Programming\\mathematicaPackages\\mathematica_notebook_\
codes\\fitBitDataAnalysis\\stepsByWeek.sql"]];
  data = Association[(#[[1]] -> #[[2]]) & /@ data];
  keys = data // Keys;
  BarChart[data,
    ChartLabels ->
